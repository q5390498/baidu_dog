...
# for filename in ["gap_train_onlyXception.h5", "gap_train_onlyResNet50.h5", "gap_train_onlyVGG16.h5"]:
#     with h5py.File(filename, 'r') as h:
#         X_train.append(np.array(h['train']))
#         y_train = np.array(h['train_label'])
...
model_save = ModelCheckpoint('blend_{val_acc:.3f}.h5')
model.fit(X_train, y, batch_size=200, epochs=200, validation_split=0.2,callbacks=[auto_lr, model_save])
model.save('model.h5')
...
model = load_model('blend_0.831.h5')
